...
class MockClient:
    ...
    @_latent
    def set_repo_visibility(self, name: str, visibility: str):
        """Flip the in-memory repo's visibility and return it (no network)."""
        if visibility not in ("public", "private", "internal"):
            raise GitHubError(f"invalid visibility {visibility!r}")
        key = name.split("/")[-1]
        repo = self._repos_by_name.get(key)
        if repo is None:
            raise GitHubError("not found", 404)
